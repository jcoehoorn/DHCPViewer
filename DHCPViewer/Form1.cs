...
namespace DHCPViewer
{
    public partial class Form1 : Form
    {
        ...
        private void dgvLeases_SortCompare(object sender, DataGridViewSortCompareEventArgs e)
        {
            ...
            //TODO: This is inefficient, requires potentially parsing the same values over and over again
            //       It doesn't seem to cause noticable delays, so low priority, but I'd still like to refactor
            //       this to re-use parsed values.
            int result = 0;
            if (e.Column.HeaderText == "IP Address")
            {
                if (string.IsNullOrEmpty(val1) && !string.IsNullOrEmpty(val2))
                {
                    result = -1;
                }
                else if (string.IsNullOrEmpty(val2) && !string.IsNullOrEmpty(val1))
                {
                    result = 1;
                }
                else if (string.IsNullOrEmpty(val2) && string.IsNullOrEmpty(val1))
